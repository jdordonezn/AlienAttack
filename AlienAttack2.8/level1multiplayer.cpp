...
void level1Multiplayer::keyPressEvent(QKeyEvent *ev)
{
    ...
    if(ev->key()==Qt::Key_Escape){  //Tecla para dar la orden de cerrar la simulación
        ...
        optionSave->setMultiplayer(true);
        optionSave->setLcd_s1(getLcd_score());
        optionSave->setLcd_h1(getLcd_health());
        optionSave->setLcd_s2(getLcd_score2());
        optionSave->setLcd_h2(getLcd_health2());
        optionSave->setEnemies(getEnemies());
        optionSave->setPlayer(getPlayer());
        optionSave->setPlayer2(getPlayer2());
        optionSave->show();
        ev->accept();
    }
